...
export async function runScrape(options) {
    ...
    for (const source of availableSources) {
        ...
        try {
            ...
            for await (const rawLead of scraper.scrape(query)) {
                ...
                if (!options.skipDeduplication) {
                    ...
                    if (duplicate) {
                        ...
                        logger.debug(`Duplicate found: "${rawLead.companyName}" matches "${duplicate.match.companyName}" (${duplicate.reason}, ${(duplicate.confidence * 100).toFixed(0)}%)`);
                        // Merge data into existing lead
                        const merged = mergeLeads(duplicate.match, rawLead);
            ...
            logger.info(`Completed ${source}: ${sourceStats.found} found, ${sourceStats.saved} saved, ${sourceStats.duplicates} duplicates`);
        }
        catch (error) {
    ...
    logger.info(`Scrape complete: ${result.totalFound} found, ${result.totalSaved} saved, ${result.totalDuplicates} duplicates`);
    return result;
}
/**
 * Get available sources that can be scraped
 */
export function getScrapableSources() {
    return getAvailableScrapers();
}
//# sourceMappingURL=scrape.orchestrator.js.map