...
export class LeadMatcher {
    fuse;
    phoneIndex = new Map();
    websiteIndex = new Map();
    sourceIdIndex = new Map();
    ...
    findDuplicate(lead) {
        ...
        for (const result of results) {
            ...
            if (confidence >= 0.7) {
                ...
                const normalizedAddressValue = 'normalizedAddress' in lead
                ...
                if (normalizedAddressValue && result.item.normalizedAddress) {
                    ...
                    const addressSimilarity = this.calculateSimilarity(normalizedAddressValue, result.item.normalizedAddress);
 ...
 */
export function mergeLeads(canonical, duplicate) {
    return {
        ...canonical,
        ...
        rating: canonical.rating !== undefined && duplicate.rating !== undefined
            ? Math.max(canonical.rating, duplicate.rating)
            : canonical.rating ?? duplicate.rating,
        reviewCount: canonical.reviewCount !== undefined && duplicate.reviewCount !== undefined
        ...
        metadata: {
            ...
            sources: [
                ...(canonical.metadata.sources ?? [canonical.source]),
                duplicate.source,
            ],
        },
    };
